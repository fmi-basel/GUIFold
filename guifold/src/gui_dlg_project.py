...
class ProjectDlg(QtWidgets.QDialog):
    ...
    def OnBtnOK(self):
        ...
        if self.prj.name.get_value() == "":
            ...
            if self.prj.check_if_name_exists(self.prj.name.value, self.sess) and not self.mode == 'update':
                message_dlg('Error', 'Project name already exists!')
                logger.debug(f"mode is {self.mode}")
            elif self.prj.check_if_path_exists(self.prj.path.value, self.sess) and not self.mode == 'update':
                message_dlg('Error', 'Project path already used in a different project!')
                logger.debug(f"mode is {self.mode}")
            else:
                if self.mode == 'add':
